...
- Do not modify `roche-pi` or sample-project in this plan. Their cutover receives a separate plan after standalone feature parity and review.
...
The local sample-project `devenv.nix` reviewed during design is ignored and untracked, so it is an operator-supplied migration input rather than an immutable source anchor. Task 15 records that limitation instead of attributing the file to a commit.
...
func TestValidateRejectsWildcardRepository(t *testing.T) {
    cfg := validConfig()
    repo := cfg.Repositories["sample-project"]
    repo.Owner = "alpha*"
    cfg.Repositories["sample-project"] = repo
...
- Produces an exact rollback-safe checklist for a later `roche-pi`/sample-project migration plan; it does not modify those repositories.
