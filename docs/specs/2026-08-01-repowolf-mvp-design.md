...
- preserve the current jailed GitHub broker's useful operation surface for sample-project;
...
repositories:
  sample-project-infra:
    provider: github-public
    owner: example-org
    name: sample-project_infra
    git:
      denyRefs:
        - refs/heads/main
      denyDeletes: true
...
principals:
  sample-project-infra-agent:
    tokenEnvs:
      - REPOWOLF_TOKEN_SAMPLE_PROJECT_INFRA
    grants:
      - repository: sample-project-infra
        capabilities:
          - repository:read
          - issues:read
          - issues:write
...
The MVP preserves the current GitHub broker's useful sample-project surface and adds safe Gitea equivalents where supported:
...
3. Add the RepoWolf client, endpoint, trusted CA, and role token to the sample-project jail.
...
9. Switch sample-project to RepoWolf while retaining a documented rollback.
...
- sample-project retains its current approved GitHub operation surface;
