...
For `include_comments`, the adapter calls Gitea's paginated issue timeline endpoint through `ListIssueTimeline` sequentially with page size 50, starting at page 1. Gitea 1.27.2's issue-comments endpoint ignores `page` and `limit`, while its issue-timeline endpoint honors both parameters. The adapter counts every timeline entry for page termination and retains only entries whose typed provider kind is `comment`; the issue's validated comment count proves that the complete comment set was recovered. A short or empty timeline page ends retrieval. At most 20 full pages (1,000 timeline entries and therefore no more than 1,000 comments) are accepted; after a twentieth full page, page 21 is requested at the same page size to distinguish exact completion from truncation. If another entry exists, the operation fails with the sanitized response-limit error rather than returning partial comments. Cancellation, deadline, any malformed comment, repeated/non-increasing comment IDs, count mismatch, or any page error stops the loop with no partial response and no retry. The comments must remain in the strictly increasing provider order returned by Gitea.
...
- Comment pagination tests cover empty/short timeline pages, comments mixed with timeline events, two pages, exactly 1,000 comments plus an empty page-21 probe, overflow detection, repeated/out-of-order IDs, cancellation between pages, and failure without partial results. Server tests enforce the aggregate Protobuf response limit, including metadata and comments.
...
Assert exact typed output, issue-only filtering in the shared index space, explicit paginated timeline requests with 50 comments on page 1 and one comment on page 2, canonical upstream owner/repository, an empty page beyond the end, denied repository behavior, accepted/completed audit events, and absence of provider credentials and issue/comment bodies from sandbox environment, process arguments, diagnostics, and audit.
