...
func TestLabelListPagination(t *testing.T) {
	...
	t.Run("empty label names are rejected", func(t *testing.T) {
		caller := &fakeCaller{results: []runner.Result{{Stdout: []byte(includedResponse(nil, `[{"name":""}]`))}}}
...
func TestLabelListRejectsPaginationMetadata(t *testing.T) {
	...
	tests := []struct {
		...
		{"wrong next page", includedResponse([]string{`Link: <https://api.github.example/labels?page=3>; rel="next"`}, validBody)},
		{"malformed relation", includedResponse([]string{`Link: https://api.github.example/labels?page=2; rel="next"`}, validBody)},
		{"duplicate relation", includedResponse([]string{`Link: <https://api.github.example/labels?page=2>; rel="next", <https://api.github.example/labels?page=2>; rel="next"`}, validBody)},
		{"short body advertises next", includedResponse([]string{`Link: <https://api.github.example/labels?page=2>; rel="next"`}, validBody)},
	}
...
func labelPage(t *testing.T, first, count int, hasNext bool) []byte {
	...
	if hasNext {
		page := (first-1)/100 + 2
		headers = []string{fmt.Sprintf(`lInK: <https://untrusted.example/labels?page=%d>; rel="next"`, page)}
	}
	return []byte(includedResponse(headers, string(body)))
}
