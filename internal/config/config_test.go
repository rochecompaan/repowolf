...
func TestValidateRejectsInvalidConfiguration(t *testing.T) {
	tests := []struct {
		...
		{"duplicate token environment", func(cfg *Config) {
			cfg.Principals["other"] = Principal{TokenEnvs: []string{"REPOWOLF_TOKEN_AGENT"}, Grants: []Grant{{Repository: "sample-project", Capabilities: []Capability{RepositoryRead}}}}
		}},
...
func TestRepositoryReturnsIndependentCopy(t *testing.T) {
	cfg := validConfig()
	repository, ok := cfg.Repository("sample-project")
	if !ok {
		t.Fatal("Repository did not find sample-project")
	}
	repository.Git.DenyRefs[0] = "refs/heads/other"
	if got := cfg.Repositories["sample-project"].Git.DenyRefs[0]; got != "refs/heads/main" {
...
func validConfig() Config {
	return Config{
		...
		Principals: map[string]Principal{
			"agent": {TokenEnvs: []string{"REPOWOLF_TOKEN_AGENT"}, Grants: []Grant{{Repository: "sample-project", Capabilities: []Capability{RepositoryRead, GitRead, GitWrite}}}},
		},
